...
3. Add `http://127.0.0.1:8888/callback` as a redirect URI — Spotify explicitly requires a loopback IP rather than `localhost`, and permits HTTP for loopback addresses
...
> **Codespaces note:** this tool works from inside a Codespace only when using **VS Code Desktop**, which tunnels forwarded ports to your local machine so `127.0.0.1:8888` resolves correctly. If using **VS Code Web**, run the tool locally instead.

Then visit `http://127.0.0.1:8888/login` in a browser. After authorizing, the callback returns a JSON response like:
